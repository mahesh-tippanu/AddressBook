...
public class AddressBook {
    ...
    public static void main(String[] args) {
        ...
        HashMap<String, AddressBook> multipleAddressBook = new HashMap<>();
        ...
        multipleAddressBook.put("AB1", addressBookObj1);
        multipleAddressBook.put("AB2", addressBookObj2);
        multipleAddressBook.put("AB3", addressBookObj3);
        while (!is_Running) {
            ...
            if (option == 4) break;
            System.out.println(" Enter 1 to create a new contact \n 2 to exit \n 3 to edit existing contact \n 4 to delete an existing contact \n 5 to sort contacts by Name\n 6 to sort contacts by city\n 7 to sort contacts by state\n 8 to sort contacts by Zip");
            int choice = scanner.nextInt();
            if (choice == 1) {
                ContactInfo contact = new ContactInfo();
                contact.setContactInfo();
                name = contact.firstName.toUpperCase(Locale.ROOT) + " " + contact.lastName.toUpperCase(Locale.ROOT);
                multipleAddressBook.get(key).addressBook.put(name, contact);
                multipleAddressBook.get(key).addressBook.get(name).displayContactInfo();
            } else if (choice == 2) {
                is_Running = true;
            } else if (choice == 3) {
                multipleAddressBook.get(key).editContact();
            } else if (choice == 4) {
                multipleAddressBook.get(key).deleteContact();
            } else if (choice == 5) {
                sorting.sortContactsByPersonName(multipleAddressBook);
            } else if (choice == 6) {
                sorting.sortContactsByCity(multipleAddressBook);
            } else if (choice == 7) {
                sorting.sortContactsByState(multipleAddressBook);
            } else if (choice == 8) {
                sorting.sortContactsByZip(multipleAddressBook);
            }
        }
        }
